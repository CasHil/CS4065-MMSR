...
def find_max_values(keywords, columns):
    ...
    for column in columns:
        ...
        for keyword in keywords:
            ...
            feeling_dict[column] = {"min": min[1],
                                    "max": max[1], "distance": max[0] - min[0]}
    return feeling_dict
...
def get_keyword_weights_normalized(keyword, distances, columns):
    ...
    for column in columns:
        ...
        if distance["distance"] == 0.0:
            ...
            normalized_weight = (
                distance["distance"] - distance_to_min) / distance["distance"]
...
def recommend_songs(song_vector):
    ...
    # extract songtitles
    song_titles = [song_title[2:-1]
                   for song_title in rows_with_min_distances['song_title']]
    return song_titles
...
def post_endpoint():
    ...
    for keyword in situation_keywords:
        keyword_weights = get_keyword_weights_normalized(
            keyword, situation_dict, columns)
        for key, weight in keyword_weights.items():
            weights_summed[key] += weight
    for keyword in sentiment_keywords:
        keyword_weights = get_keyword_weights_normalized(
            keyword, sentiment_dict, columns)
        for key, weight in keyword_weights.items():
            weights_summed[key] += weight
    # normalize
    for weight in weights_summed:
        if len(situation_keywords) > 0:
            weights_summed[weight] = weights_summed[weight] / \
...
def prompt_user():
    ...
    situation_keywords = input(
        "Input some keywords related to the situation comma seperated: ").split(",")
    sentiment_keywords = input(
        "Input some keywords related to the sentiment/feelings comma seperated: ").split(",")
    for sentiment_keyword in sentiment_keywords:
        if sentiment_keyword not in sentiment_list:
            print(
                "You used a sentiment keyword not in the list, please do try agian good sir")
            return [], []
    for situation_keyword in situation_keywords:
        if situation_keyword not in situation_keywords:
            print(
                "You used a situation keyword not in the list, please do try agian good sir")
            return [], []
    return (situation_keywords, sentiment_keywords)
